...
def get_contractual(parque, componente_obj, estado, fecha):
...
def get_plan(parque, componente_obj, estado, fecha):
...
def get_real(parque, componente_obj, estado, fecha):
...
def grafico_componentes(parque_eolico, estado, fecha_calculo):
    data_full = []
    ...
    for m in miembros:
        value = get_contractual(parque_eolico.parque, m.componente, estado, r)
    ...
    data_graficos = []
    for m in miembros:
        value = get_plan(parque_eolico.parque, m.componente, estado, r)
        data_graficos.append({"name": m.componente.nombre, "y": value})
    ...
    data_graficos = []
    for m in miembros:
        value = get_real(parque_eolico.parque, m.componente, estado, r)
        data_graficos.append({"name": m.componente.nombre, "y": value})
...
def calcular_proyeccion(parque_eolico, anho, semana):
    ...
    ag_montar = aerogeneradores_a_montar(parque_eolico)
    ...
    # Calculo de avance
    fecha = configuracion_obj.fecha_inicio
...
def calcular_proyeccion_grafico(parque_eolico, anho, semana):
    d = str(anho) + '-W' + str(semana)
    # Se calcula solo con los datos de hasta la semana pasada
    r = datetime.strptime(d + '-0', "%Y-W%W-%w") - relativedelta.relativedelta(weeks=1)
    parque = parque_eolico.parque
...
def aerogeneradores_a_montar(parque_eolico):
    estado_montaje = EstadoFU.objects.get(nombre='Montaje')
...
def aerogeneradores_montados(parque_eolico, fecha):
    parque = parque_eolico.parque
    ...
    cuenta_total = aerogeneradores_a_montar(parque_eolico)
    miembros = Membership.objects.filter(parque_eolico=parque_eolico,
                                         estado=estado_montaje,
                                         orden=cuenta_total)
    try:
        componente_obj = miembros[0].componente
    except:
        return 0
...
def porcentaje_avance(parque_eolico, fecha, componentes_montaje=None):
    parque = parque_eolico.parque
    max_aerogeneradores = parque.no_aerogeneradores
    estado_montaje = EstadoFU.objects.get(nombre='Montaje')
    miembros = None
...
def grafico_avances(parque_eolico, anho, semana, actual_date, data_proyeccion):
    data_full = []
    d = str(anho) + '-W' + str(semana)
    ...
    while fecha_calculo <= r2:
        if fecha_calculo == r2:
            valor = porcentaje_avance(parque_eolico, actual_date, componentes_montaje=componentes_montaje)
        else:
            valor = porcentaje_avance(parque_eolico, fecha_calculo, componentes_montaje=componentes_montaje)
        fecha_grafico = str(fecha_calculo.year) + '-' + str(semana_calculo)
        data_graficos.append({"name": fecha_grafico, "y": valor})
        fecha = fecha_calculo + relativedelta.relativedelta(weeks=1)
        semana_calculo = fecha.isocalendar()[1]
        ...
        if len(pos[ag.nombre]) > 0:
            ...
            if registros.count() > 0:
                ...
                for m in miembros:
                    ...
                    filename = path + '/' + str(m.componente.nombre) + '.png'
                    if os.path.isfile(str(filename).encode('utf-8')):
                        found = True
            ...
            # Inicio Pruebas
            # path = os.path.join(settings.BASE_DIR, 'static/common/images/ag')
            # # filename = path + '/' + str(r.componente.nombre) + '.png'
            # # filename = path + '/' + str('T1') + '.png'
            # filename = path + '/' + str('T1') + '.png'
            # if os.path.isfile(str(filename).encode('utf-8')):
            #     found = True
            #     if parque.codigo in ['ARCO-002.1', 'ARCO-002.2']:
...
def dashboard(request, slug):
    ...
    graficoDescarga = grafico_componentes(parque_eolico, estado, last_day_week)
    estado = EstadoFU.objects.get(idx=3)
    graficoMontaje = grafico_componentes(parque_eolico, estado, last_day_week)
    estado = EstadoFU.objects.get(idx=4)
    graficoPuestaenMarcha = grafico_componentes(parque_eolico, estado, last_day_week)
    [proyeccion, last_week] = calcular_proyeccion_grafico(parque_eolico, anho, semana)
    if last_week is not None:
        graficoAvance = grafico_avances(parque_eolico, last_week.isocalendar()[0], last_week.isocalendar()[1],
                                        last_day_week, proyeccion)
    else:
        ...
        fecha_calculo = datetime.strptime(d + '-0', "%Y-W%W-%w")
        graficoAvance = grafico_avances(parque_eolico, fecha_calculo.isocalendar()[0], fecha_calculo.isocalendar()[1],
                                        last_day_week,
                                        proyeccion)
    ...
    avance = porcentaje_avance(parque_eolico, last_day_week)
    montados = aerogeneradores_montados(parque_eolico, last_day_week)
    try:
...
def dashboard_diario(request, slug):
    ...
    estado = EstadoFU.objects.get(idx=1)
    graficoDescarga = grafico_componentes(parque_eolico, estado, t)
    estado = EstadoFU.objects.get(idx=3)
    graficoMontaje = grafico_componentes(parque_eolico, estado, t)
    estado = EstadoFU.objects.get(idx=4)
    graficoPuestaenMarcha = grafico_componentes(parque_eolico, estado, t)
    [proyeccion, last_week] = calcular_proyeccion_grafico(parque_eolico, anho, semana)
    if last_week is not None:
        graficoAvance = grafico_avances(parque_eolico,
...
def componente(request, slug):
    ...
    if request.method == 'POST':
        if 'delete' in request.POST:
            ...
            input_form = ComponenteForm(request.POST)
            if input_form.is_valid():
                c.nombre = input_form.cleaned_data['nombre']
                c.estados.clear()
                for e in input_form.cleaned_data['estadofu']:
                    c.estados.add(e)
                c.save()
            ...
            if input_form.is_valid():
                ...
                c = Componente(nombre=input_form.cleaned_data['nombre'])
                c.save()
                for e in input_form.cleaned_data['estadofu']:
                    c.estados.add(e)
                c.save()
    ...
    return TemplateResponse(request, 'fu/componentes.html',
                            {'cont': contenido,
                             'parque': parque,
                             'aerogeneradores': aerogeneradores,
                             'componenteForm': componente_form,
                             'componentes': componentes,
                             })


def get_orden(componente_obj, d, p, m, pm):
    ret_d = 0
    ret_p = 0
    ret_m = 0
    ret_pm = 0
    for e in componente_obj.estados.all():
        if e.idx == 1:
            ret_d = d + 1
        elif e.idx == 2:
            ret_p = p + 1
...
def get_componentes_by_state(parque_eolico, estado):
    if estado == 'descarga':
    ...
    elif estado == 'obracivil':
        e = EstadoFU.objects.get(nombre='Obra Civil')
    else:
        return None
...
def add_componente(parque_eolico, new_componente, estados):
...
def delete_componente(parque_eolico, del_componente):
    ...
    #         c.save()
    # aux.delete()
    # fix_orden_componentes()


def fix_orden_componentes(parque_eolico):
...
def actividades(request, slug):
    ...
    if request.method == 'POST':
        if 'delComponente' in request.POST:
            delete_componentes_form = DeleteComponentesForm(request.POST, parque=parque)
            if delete_componentes_form.is_valid():
                log_msg = "Se elimina componente del proyecto (actividad) para parque " + parque.nombre + \
                          " - Nombre " + delete_componentes_form.cleaned_data['componente'].nombre

                delete_componente(parque_eolico, delete_componentes_form.cleaned_data['componente'])

                log = Log(texto=log_msg, tipo=3, user=request.user)
                log.save()
        elif 'addComponente' in request.POST:
            choices_componentes_form = AddComponentesForm(request.POST, parque=parque)
            if choices_componentes_form.is_valid():
                log_msg = "Se agrega componente al proyecto (actividad) para parque " + parque.nombre + \
                          " - Nombre " + choices_componentes_form.cleaned_data['componente'].nombre

                add_componente(parque_eolico,
                               choices_componentes_form.cleaned_data['componente'],
                               choices_componentes_form.cleaned_data['estadofu'])
                log = Log(texto=log_msg, tipo=1, user=request.user)
                log.save()

    choices_componentes_form = AddComponentesForm(parque=parque)
    delete_componentes_form = DeleteComponentesForm(parque=parque)

    lista_obracivil = get_componentes_by_state(parque_eolico, 'obracivil')
    lista_descarga = get_componentes_by_state(parque_eolico, 'descarga')
    lista_premontaje = get_componentes_by_state(parque_eolico, 'premontaje')
    lista_montaje = get_componentes_by_state(parque_eolico, 'montaje')
    lista_puestaenmarcha = get_componentes_by_state(parque_eolico, 'puestaenmarcha')

    actividades_dict = OrderedDict()
    actividades_dict['obracivil'] = lista_obracivil
    actividades_dict['descarga'] = lista_descarga
    actividades_dict['premontaje'] = lista_premontaje
    actividades_dict['montaje'] = lista_montaje
    actividades_dict['puestaenmarcha'] = lista_puestaenmarcha
    ...
    return TemplateResponse(request, 'fu/actividades.html',
                            {'cont': contenido,
                             'parque': parque,
                             'aerogeneradores': aerogeneradores,
                             'choicesComponentesForm': choices_componentes_form,
                             'deleteComponentesForm': delete_componentes_form,
...
def check_valid_file(ws, parque_eolico):
    fila = 4
    columna = 2
...
def read_plan_file(configuracion, parque_eolico):
    nombre_archivo = os.path.join(settings.MEDIA_ROOT, configuracion.plan.name)
    wb = load_workbook(nombre_archivo)
    ws = wb.active
    last_row = check_valid_file(ws, parque_eolico)
    if last_row == -1:
        ...
        for obj in objs:
            obj.delete()
    last_column = get_last_column_v2(ws)
    fila = 4
    ultimo_estado = None
    ultimo_anho = None
    estado = None
    while fila < last_row:
        ...
        # lo puedo hacer de forma segura, porque se supone que los validé en check_valid_file
...
def grafico_planificacion(parque):
    configuracion = ConfiguracionFU.objects.get(parque=parque)
    parque_eolico = ParqueEolico.objects.get(parque=parque)
...
def planificacion(request, slug):
    ...
    if request.method == 'POST':
        ...
        form = PlanificacionForm(request.POST, request.FILES, instance=configuracion_obj)
        if form.is_valid():
            configuracion_obj = form.save()

        if configuracion_obj.plan:
            if configuracion.plan != configuracion_obj.prev_plan:
                read_plan_file(configuracion_obj, parque_eolico)
    ...
    if form is None and configuracion_obj is not None:
        form = PlanificacionForm(instance=configuracion_obj)

    if configuracion_obj:
        [x_axis, y_axis, plan, contractual] = grafico_planificacion(parque)
    else:
        x_axis = None
        y_axis = None
        plan = None
    ...
    return TemplateResponse(request, 'fu/planificacion.html',
                            {'cont': contenido,
                             'parque': parque,
                             'aerogeneradores': aerogeneradores,
                             'configuracion': configuracion_obj,
                             'form': form,
                             'x_axis': x_axis,
                             'y_axis': y_axis,
                             'plan': plan,
...
def download_config(request, slug):
    ...
    aux = configuracion_obj.fecha_inicio
    final = configuracion_obj.fecha_final
    ...
    for s in semanas:
        ...
        if prev_year != anhos[semanas[idx]]:
            ...
            if prev_year_col == 0:
                ...
                ws.merge_cells(start_row=row - 2, start_column=prev_year_col, end_row=row - 2,
                               end_column=column + idx - 1)
                prev_year_col = column + idx
        ...
        if prev_month != meses[semanas[idx]]:
            ...
            if prev_month_col == 0:
                ...
                ws.merge_cells(start_row=row - 1, start_column=prev_month_col, end_row=row - 1,
                               end_column=column + idx - 1)
                prev_month_col = column + idx
        idx += 1
    ...
    for e, titulo in estados.items():
        componentes = get_componentes_by_state(parque_eolico, e)
        if len(componentes) > 0:
            ...
            first_row = row
            first = True
            for idx, nombre in componentes.items():
...
def get_componente_status(registros, idx, componente, miembros):
    aux = registros.filter(componente=componente, estado__idx=idx)
    if aux.count() > 0:
        return 2
    if idx == 5:  # Estado Obra Civil
        m_obj = miembros.get(componente=componente)
        # miembros mas bajos en el orden
        miembros_list = miembros.filter(estado__idx=idx, orden__lt=m_obj.orden)
        miembros_menores = list()
        for m in miembros_list:
            miembros_menores.append(m.componente)
        # registros de los componentes menores
        cuenta_registrados = registros.filter(estado__idx=idx, componente__in=miembros_menores).count()
        if cuenta_registrados == len(miembros_menores):
            return 0
        else:
            return 1
    elif idx == 1:  # Estado descarga
        return 0
        ...
        if prerequisitos.count() > 0:
            ...
            for prerequisito in prerequisitos:
                ...
                if prerequisito.estado.idx != 2:
                    test = test and registros.filter(componente=componente,
                                                     estado__idx=prerequisito.estado.idx).exists()
...
def ingreso(request, slug, slug_ag):
    ...
    if request.method == 'POST':
        registro = None
        if 'formObraCivil' in request.POST:
            if not request.user.has_perm('fu.add_registros'):
                raise PermissionDenied
            form = RegistroForm(request.POST)
            if form.is_valid():
                registro = form.save(commit=False)
                estado = EstadoFU.objects.get(idx=5)
            else:
                messages.add_message(request, messages.ERROR, 'Registro no pudo realizarse')
        elif 'formDescarga' in request.POST:
            if not request.user.has_perm('fu.add_registros'):
                raise PermissionDenied
            form_descarga = RegistroDescargaForm(request.POST)
            if form_descarga.is_valid():
                registro = form_descarga.save(commit=False)
                estado = EstadoFU.objects.get(idx=1)
            else:
                messages.add_message(request, messages.ERROR, 'Registro no pudo realizarse')
        ...
        if registro is not None:
            ...
            componente_obj = Componente.objects.get(id=componente_id)
            registro.componente = componente_obj
    ...
    miembros = Membership.objects.filter(parque_eolico=parque_eolico)
    miembros_civiles = miembros.filter(estado__nombre='Obra Civil').count()
    componentes = OrderedDict()
    if miembros_civiles > 0:
        componentes['Obra Civil'] = {}
    ...
    if miembros_civiles > 0:
        componentes['Obra Civil']['objetos'] = list()
        # filtro = 'relacionesfu__orden_descarga'
        # karws = {filtro + '__gt': 0}
        estado = EstadoFU.objects.get(nombre='Obra Civil')
        for m in miembros.filter(estado=estado).order_by('orden'):
            aux = get_componente_status(registros, 5, m.componente, miembros)
            color = 'bg-grey-salt'
            objeto = {}
            # 2: Ya está ingresado.
            if aux == 2:
                color = 'bg-green-meadow'
                reg = registros.get(componente=m.componente, estado__idx=5)
            ...
            objeto['status'] = aux
            objeto['estado'] = 5
            componentes['Obra Civil']['objetos'].append(objeto)

    componentes['Descarga en Parque']['objetos'] = list()
    # filtro = 'relacionesfu__orden_descarga'
    # karws = {filtro + '__gt': 0}
    estado = EstadoFU.objects.get(nombre='Descarga')
    for m in miembros.filter(estado=estado).order_by('orden'):
        aux = get_componente_status(registros, 1, m.componente, miembros)
        objeto = {}
        color = 'bg-yellow-crusta'
        if aux == 2:
            color = 'bg-green-meadow'
    ...
    componentes['Pre-montaje']['objetos'] = list()
    # filtro = 'relacionesfu__orden_montaje'
    # karws = {filtro + '__gt': 0}
    estado = EstadoFU.objects.get(nombre='Pre-Montaje')
    for m in miembros.filter(estado=estado).order_by('orden'):
        aux = get_componente_status(registros, 2, m.componente, miembros)
        color = 'bg-grey-salt'
        objeto = {}
        if aux == 2:
            color = 'bg-green-meadow'
    ...
    componentes['Montaje']['objetos'] = list()
    # filtro = 'relacionesfu__orden_montaje'
    # karws = {filtro + '__gt': 0}
    estado = EstadoFU.objects.get(nombre='Montaje')
    for m in miembros.filter(estado=estado).order_by('orden'):
        aux = get_componente_status(registros, 3, m.componente, miembros)
        color = 'bg-grey-salt'
        objeto = {}
        if aux == 2:
            color = 'bg-green-meadow'
    ...
    componentes['Puesta en marcha']['objetos'] = list()
    # filtro = 'relacionesfu__orden_puestaenmarcha'
    # karws = {filtro + '__gt': 0}
    estado = EstadoFU.objects.get(nombre='Puesta en marcha')
    for m in miembros.filter(estado=estado).order_by('orden'):
        aux = get_componente_status(registros, 4, m.componente, miembros)
        color = 'bg-grey-salt'
        objeto = {}
        if aux == 2:
            color = 'bg-green-meadow'
    ...
    if miembros_civiles > 0:
        componentes['Obra Civil']['id'] = 'obracivil'
    ...
    if miembros_civiles > 0:
        componentes['Obra Civil']['icon'] = 'fa-cubes'
    ...
    form_descarga = RegistroDescargaForm()
    form = RegistroForm()
...
def paradas(request, slug):
    ...
    paradas_obj = Paradas.objects.filter(parque=parque)

    if request.method == 'POST':
        if 'del_id' in request.POST:
            parada_id = int(request.POST['del_id'])
            parada = Paradas.objects.get(id=parada_id)
...
@login_required(login_url='ingresar')
def edit_paradas(request, slug, parada_id):
    parque = get_object_or_404(ParqueSolar, slug=slug)
    aerogeneradores = Aerogenerador.objects.filter(parque=parque).order_by('idx')
    parada = get_object_or_404(Paradas, id=parada_id)
    contenido = ContenidoContainer()
    contenido.user = request.user
    contenido.titulo = u'Listado de Paradas '
    contenido.subtitulo = 'Parque ' + parque.nombre
...
def nested_dict(n, tipo):
    if n == 1:
        return defaultdict(tipo)
    else:
        return defaultdict(lambda: nested_dict(n - 1, tipo))


def data_planificacion(parque):
    parque_eolico = ParqueEolico.objects.get(parque=parque)
...
def print_filas_header(ws, node, row, column, deep, alignment1=None):
    deep += 1
    bgColor = Color(rgb="283861")
    bg = PatternFill(patternType='solid', fgColor=bgColor)
    font = Font(color="FFFFFF")
    ...
    for level in node.children:
        ...
        if len(level.children) > 0:
            row = print_filas_header(ws, level, row, column, deep)
        else:
            # d.border = borderfull
            row += 1
        # d.border = borderbottom
        if first_row != (row - 1):
            ws.merge_cells(start_row=first_row, start_column=column + deep - 1, end_row=row - 1,
                           end_column=column + deep - 1)
        else:
            d.border = borderfull
...
def print_columnas_header(ws, node, row, column, deep):
    deep += 1
    bgColor = Color(rgb="283861")
    bg = PatternFill(patternType='solid', fgColor=bgColor)
    font = Font(color="FFFFFF")
    ...
    for level in node.children:
        ...
        start_column = column
        if len(level.children) > 0:
            column = print_columnas_header(ws, level, row, column, deep)
        else:
            column += 1
...
def get_hojas(node, hojas):
    if len(node.children) == 0:
        hojas.append(node)
    else:
        for level in node.children:
            get_hojas(level, hojas)


def print_data_excel(ws, datos, initial_row, initial_col, alignment=None, column_width=None):
    thin = Side(border_style="thin", color="000000")
    borderbottom = Border(left=thin, right=thin, bottom=thin)

    max_row = 0
    max_col = 0
    for fila, aux in datos.items():
        if fila > max_row:
            max_row = fila
        for columna, value in aux.items():
            if columna > max_col:
                max_col = columna
...
def planificacion_excel(parque, wb):
    [filas, columnas, datos] = data_planificacion(parque)
    ...
    print_columnas_header(ws, columnas, 1, 4, 0)
    print_filas_header(ws, filas, 4, 1, 0)
    ...
    print_data_excel(ws, datos, 4, 4)

    return wb


def add_unique_nodo(nombre, padre, html=None):
    r = Resolver('name')
    try:
...
def data_series_fechas(parque, html=None):
    datos = OrderedDict()
    filas = Node("Filas")
    columnas = Node("Columnas")
    parque_eolico = ParqueEolico.objects.get(parque=parque)
    ...
    for e in estados:
        ...
        if miembros.count() > 0:
            ...
            for m in miembros:
                ...
                columna = 0
                for ag in ags:
                    add_unique_nodo(ag.nombre, columnas, columnas_html)
                    try:
                        ...
                        pass
                    columna += 1
                add_unique_nodo('Total', columnas, columnas_html)
...
def series_fechas_excel(parque, wb):
    [filas, columnas, datos] = data_series_fechas(parque)
    ...
    print_columnas_header(ws, columnas, 2, 4, 0)
    print_filas_header(ws, filas, 3, 1, 0)
    for cell_range in ws.merged_cell_ranges:
        rows = ws[cell_range.coord]
        if len(rows) > 1:
            for row in rows:
    ...
    print_data_excel(ws, datos, 3, 4, alignment3, 14)
...
def data_seguimiento(parque, t, html=None):
    datos = OrderedDict()
    filas = Node("Filas")
    columnas = Node("Columnas")
    parque_eolico = ParqueEolico.objects.get(parque=parque)
    ...
    for e in estados:
        ...
        if miembros.count() > 0:
            ...
            for m in miembros:
                ...
                columna += 1
                add_unique_nodo('Contractual', columnas, columnas_html)
                datos[fila][columna] = get_contractual(parque, m.componente, e, fecha_calculo)
                columna += 1
                add_unique_nodo('Plan', columnas, columnas_html)
                datos[fila][columna] = get_plan(parque, m.componente, e, fecha_calculo)
                columna += 1
                add_unique_nodo('Real', columnas, columnas_html)
                datos[fila][columna] = get_real(parque, m.componente, e, t)
                columna += 1
...
def seguimiento_excel(parque, wb, t):
    [filas, columnas, datos] = data_seguimiento(parque, t)
    ...
    print_columnas_header(ws, columnas, 2, 3, 0)
    print_filas_header(ws, filas, 3, 1, 0)
    for cell_range in ws.merged_cell_ranges:
        rows = ws[cell_range.coord]
        if len(rows) > 1:
    ...
    print_data_excel(ws, datos, 3, 3, alignment3, 6)
...
def data_tasa_montaje(parque, t, html=None):
    datos = OrderedDict()
    ...
    n_componentes = aerogeneradores_a_montar(parque_eolico)

    [proyeccion, ultima_fecha] = calcular_proyeccion(parque_eolico, t.isocalendar()[0], t.isocalendar()[1])
    ...
    while aux < final:
        if last_anho != aux.year:
            n_anho = add_unique_nodo(str(aux.year), filas)
            last_anho = aux.year
        if last_mes != aux.month:
            n_mes = add_unique_nodo(meses_espanol[str(aux.month)], n_anho)
            last_mes = aux.month
        n_semana = add_unique_nodo(str(aux.isocalendar()[1]), n_mes)
        datos[fila] = OrderedDict()
        ...
        if miembros.count() > 0:
            for m in miembros:
                if aux <= fecha_informe:
                    n_comp = add_unique_nodo(m.componente.nombre, columnas, columnas_html)
                    value = get_real(parque, m.componente, e, fecha_aux)
        ...
        n_comp = add_unique_nodo('Total Semana', columnas, columnas_html)
        if aux <= fecha_informe:
            datos[fila][columna] = round(float(suma_fila[fila]) / float(miembros.count()), 2)
        columna += 1

        n_comp = add_unique_nodo('Real', columnas, columnas_html)
        if aux <= fecha_informe:
            if fila > 0:
        ...
        n_comp = add_unique_nodo('Contractual', columnas, columnas_html)
        datos[fila][columna] = get_contractual(parque, m.componente, e, fecha_aux)
        columna += 1

        n_comp = add_unique_nodo('Plan', columnas, columnas_html)
        datos[fila][columna] = get_plan(parque, m.componente, e, fecha_aux)
        columna += 1

        n_comp = add_unique_nodo('Proyección', columnas, columnas_html)
...
def tasa_montaje_excel(parque, wb, t):
    parque_eolico = ParqueEolico.objects.get(parque=parque)
    [filas, columnas, datos] = data_tasa_montaje(parque, t)
    # componentes_parque = ComponentesParque.objects.get(parque=parque).componentes.all()
    miembros = Membership.objects.filter(parque_eolico=parque_eolico)
    n_componentes = aerogeneradores_a_montar(parque_eolico)
    ...
    print_data_excel(ws, datos, 3, 4, alignment3, 10)

    for col in range(12, 12 + 6):
        ws.column_dimensions[get_column_letter(col)].width = 12

    last_row = list(datos.keys())[-1] + 3 + 1
    last_col = list(datos[0].keys())[-1] + 4 - 5 + 1
...
def data_listado_paradas(parque, t, html=None):
    datos = OrderedDict()
    filas = Node("Filas")
    columnas = Node("Columnas")
    ...
    for parada in paradas:
        datos[fila] = OrderedDict()
        n_row = add_unique_nodo(str(parada.id), filas)
        n_col = add_unique_nodo('WTG', columnas, columnas_html)
        datos[fila][0] = parada.aerogenerador.nombre
        n_col = add_unique_nodo('Componente', columnas, columnas_html)
        if parada.componente is not None:
            datos[fila][1] = parada.componente.nombre
        else:
            datos[fila][1] = ''
        n_col = add_unique_nodo('Trabajo', columnas, columnas_html)
        datos[fila][2] = parada.trabajo.nombre
        n_col = add_unique_nodo('Hora Paralización', columnas, columnas_html)
        datos[fila][3] = localtime(parada.fecha_inicio).strftime('%d-%m-%Y %H:%M')
        n_col = add_unique_nodo('Hora fin Paralización', columnas, columnas_html)
        datos[fila][4] = localtime(parada.fecha_final).strftime('%d-%m-%Y %H:%M')
        n_col = add_unique_nodo('Duración Paralización (h)', columnas, columnas_html)
        datos[fila][5] = parada.duracion
        n_col = add_unique_nodo('Motivo', columnas, columnas_html)
        datos[fila][6] = parada.motivo
        n_col = add_unique_nodo('Grúa', columnas, columnas_html)
        if parada.grua is not None:
            datos[fila][7] = parada.grua.nombre
        else:
            datos[fila][7] = ''
        n_col = add_unique_nodo('Observaciones', columnas, columnas_html)
...
def listado_paradas_excel(parque, wb, t):
    [filas, columnas, datos] = data_listado_paradas(parque, t)
    ...
    print_columnas_header(ws, columnas, 2, 2, 0)
    print_filas_header(ws, filas, 3, 1, 0, Alignment(horizontal="center"))
    for cell_range in ws.merged_cell_ranges:
        rows = ws[cell_range.coord]
        if len(rows) > 1:
            for row in rows:
    ...
    print_data_excel(ws, datos, 3, 2, alignment3, 6)
...
def graficos_fu_excel(parque, wb):
    # filtros = {}
    ...
    for e in estados:
        ...
        if miembros.count() > 0:
            ...
            data1 = Reference(ws2,
                              min_col=3,
                              max_col=6,
                              min_row=3 + cuenta_anterior,
                              max_row=3 + cuenta_anterior + cuenta - 1)
            categs = Reference(ws2,
                               min_col=2,
                               max_col=2,
                               min_row=3 + cuenta_anterior,
                               max_row=3 + cuenta_anterior + cuenta - 1)
...
def reporte_excel_fu(parque, t, nombre):
    if parque.excel_fu:
        wb = load_workbook(parque.excel_fu.file.name)
    else:
        wb = Workbook()
    # wb = planificacion_excel(parque, wb)
    wb = series_fechas_excel(parque, wb)
    wb = seguimiento_excel(parque, wb, t)
    wb = tasa_montaje_excel(parque, wb, t)
    wb = listado_paradas_excel(parque, wb, t)
    wb = graficos_fu_excel(parque, wb)
...
def reporte_pdf_fu(parque, t):
    with open(os.path.join(settings.BASE_DIR, 'static/common/images/saroenlogo.png'), "rb") as image_file:
        logo_saroen = str(base64.b64encode(image_file.read()), 'utf8')

    # data Series y Fechas
    [resultados, columnas, datos] = data_series_fechas(parque, True)
    ...
    # data Seguimiento
    [filas_seguimiento, col_seguimiento, seguimiento] = data_seguimiento(parque, t, True)
    paginas_seguimiento = {}
    ag_per_page = 14
    page = 0
    ...
    [filas_tasamontaje, col_tasamontaje, tasamontaje] = data_tasa_montaje(parque, t, True)
    paginas_tasamontaje = {}
    col_per_page = 14
    page = 0
    ...
    [filas_paradas, col_paradas, paradas_list] = data_listado_paradas(parque, t, True)
