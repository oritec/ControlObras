...
class ActividadDR(models.Model):
    ...
    composicion = models.CharField(max_length=10, unique=False)
...
def dr_path(instance, filename):
    ...
    return 'dr/dr_{0}/actividad_{1}/{2}'.format(instance.actividad.dr.id,
                                      instance.actividad.id,
                                      filename)

def thumbnail_dr_path(instance, filename):
    # file will be uploaded to MEDIA_ROOT/user_<id>/<filename>
    return 'dr/dr_{0}/actividad_{1}/thumbnails/{2}'.format(instance.actividad.dr.id,
                                                instance.actividad.id,
                                                filename)
